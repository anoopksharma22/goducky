...
//        log.info("Executing " + javaCode);
        ...
        try {
            ...
            CompilationOutput compilationResult = compile(path);
    ...
    public CompilationOutput compile(Path path) {
...
//        return compiler.run(null, null, null, path.toString()) == 0;
        if (result == 0) {
            return new CompilationOutput(true,outputStream.toString());
        } else {
            return new CompilationOutput(false,errorStream.toString());
        }
    }
